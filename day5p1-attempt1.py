...
def validate(dict):
    ...
    #print("byr should be between 1920 and 2002")
    #print(dict['byr'])
    #this could be faster with a loop that breaks when a field fails
    if int(dict['byr']) < 1920 or int(dict['byr']) > 2002:
        #print("this is not valid")
        valid = 1
    #print("should be 2010-2020")
    #print(dict['iyr'])
    ...
    if "cm" in dict['hgt']:
        ...
        temp=temp.strip('cm')
        #print(temp)
        if int(temp) < 150 or int(temp) > 193:
          #  print("This is not valid")
            valid = 1
    ...
    if len(dict['hcl']) == 7:
        ...
        for i in dict['hcl'][1:]:
            if i not in validhex:
                print(i + "is not in valid")
                valid=1
    if len(dict['hcl']) != 7:
        valid = 1

    eyeColors=('amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth')
    if dict['ecl'] not in eyeColors:
        print("this is not valid")
        print(dict['ecl'])
        valid = 1
    if len(dict['pid']) != 9:
        ...
        print("this is not valid")
        valid = 1
    ##TODO validate based on the rules, each value
     
    
    if valid==0:
        return True

##ABANDOMTODO normalize file.. perhaps replace newline with --- or something for easier viewing, organizing values, etc.

#file=open("day4-input.sample2","r")
